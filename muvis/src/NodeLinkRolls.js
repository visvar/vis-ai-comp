...
const mode = 'samePitchRangePerLayer'
...
class NodeLinkRolls extends React.Component {
  constructor (props) {
    super(props)
  ...
  render () {
    try {
      ...
      const heightBox = this.props.height + margin.bottom
    ...
    */
      ...
      if (loaded && notes1.length < 50 && display) {
        if (!once) {
          d3.select('#nodeLink').on('wheel', (e) => resizeByWheel(e), { passive: false })
        }
        once = true
      ...
      */
        ...
        */
        ...
        function PlayShort (notes, i, parents, axis) {
          if (player !== undefined) {
            if (!player.isPlaying()) {
              const bpm = getBPM()

              function showNode (current, i, duration, max, min, first, axis) {
                const padding = (current.x1 - current.x0) / 40
                const xScale = d3.scaleLinear()
                  .domain([axis[current.layer].lmin, axis[current.layer].lmax])
                  .range([current.x0 + padding, current.x1 - padding])
                if (first) {
                  d3.select('#playback_line_sankey')
                    .raise()
                    .transition()
                    .duration(0) // (60000) / (bpm * 4))
                    .attr('transform', `translate(${xScale(min)},${current.y0})`)
                    .attr('height', current.y1 - current.y0)
                    .on('end', () => { play(current, i, duration, max, min, first, padding, axis) })
                } else {
                  d3.select('#playback_line_sankey')
                    .raise()
                    .attr('transform', `translate(${xScale(min)},${current.y0})`)
                  ...
                  play(current, i, duration, max, min, first, padding, axis)
                }

                function play (current, i, duration, max, min, first, padding, axis) {
                  duration++
                  if (player.isPlaying()) {
                    const xScale = d3.scaleLinear()
                      .domain([axis[current.layer].lmin, axis[current.layer].lmax])
                    ...
                    d3.select('#playback_line_sankey').transition().ease(d3.easeLinear)
                      .duration((60000) / (bpm * 4) * (duration))
                      .attr('transform', `translate(${xScale(max)},${current.y0})`)
                      .on('start', () => {
                      ...
                      .on('end', () => {
                        ...
                        setTimeout(() =>
                          showNode(parents[i], i, dataLength(transformToStart(parents[i].notes)), dataLength(parents[i].notes), dataStart(parents[i].notes), false, axis),
                        (bpm * 1000) / (60 * 16) * (diff))
                      })
                  }
                }
              ...
              player.loadSamples(seq).then(() => {
                ...
                showNode(parents[j], j, dataLength(transformToStart(parents[j].notes)), dataLength(parents[j].notes), dataStart(parents[j].notes), true, axis)
              })
            } else {
              player.stop()
              d3.select('#playback_line_sankey').attr('opacity', 0)
              ...
              parents.forEach((node) => {
                d3.select('#Node' + previousIDR)
                  .attr('stroke-width', 1)
                  .attr('stroke', 'grey')
              })
              if (previousIDR !== parents[i].idr) {
                PlayShort(notes, i, parents, axis)
              }
            }
          } else {
        ...
        // first calc all parents then set but look for children to keep
          ...
          console.log(!newfill[j])

          callback(tt, false) //!newfill[j])
        }
            ...
            if (currentIndizes.length > 0) {
              if (!player.isPlaying()) {
                ...
                player.loadSamples(seq).then(() => {
                  ...
                  d3.select('#playback_line_nlselect')
                ...
                player.stop()
                d3.select('#playback_line_nlselect').attr('opacity', 0)
              }
            }
          } else {
            player = new mm.SoundFontPlayer('https://storage.googleapis.com/magentadata/js/soundfonts/salamander')
        ...
        function showButtons () {
          const g = d3.select('#selectOfNL').append('g').attr('id', 'buttons')
          ...
          g.append('text').text('Play')
            .attr('class', 'pointer')
            .attr('fill', 'black').attr('id', 'playbuttonicicle')
        ...
        function createRollForSelect (indizes) {
          d3.select('#selectOfNL').selectAll('*').remove()
          const svg = d3.select('#selectOfNL')
          ...
          svg.append('rect')
            .attr('id', 'playback_line_nlselect')
          ...
          function changeBrush (e) {
            if (e !== undefined && e.sourceEvent !== undefined) {
              ...
              if (extent !== null) {
                ...
                d3.select('#brushSank').transition().call(e.target.move, [x(brushmin), x(brushmax)])
                brushrange = [brushmin, brushmax]
                brushactive = true
              } else {
                d3.select('#brushSank').transition().call(e.target.clear)
                brushactive = false
              }
            }
          }

          d3.select('#selectOfNL')
            .on('mouseenter', showButtons)
            .on('mouseleave', deleteButtons)

          svg.append('g').attr('id', 'brushSank').call(
            d3.brushX()
        ...
        function showButtonSankey (d, svg, newfill, axis) {
          const g = svg.append('g').attr('id', 'buttonSankey')
          svg.on('contextmenu', (e) => {
            e.preventDefault()
            if (e.button === 2) {
              PlayShort(d.notes, 0, [d], axis)
            }
          })
          ...
          g.append('rect')
            ...
            .attr('class', 'button pointer')
            .attr('x', ((d.x1 - d.x0) * 0.7) + d.x0)
            .attr('y', d.y0)
            .attr('width', (d.x1 - d.x0) * 0.3)
            .attr('height', margin.top)
            .attr('fill', 'grey')
            .attr('rx', 15)
            .attr('ry', 15)
            .on('click', () => PlayShort(d.notes, 0, [d], axis))

          g.append('text').text('Play').attr('class', 'pointer').attr('fill', 'white').attr('id', 'playbuttonsankey')
            .attr('x', ((d.x1 - d.x0) * 0.7) + d.x0 + ((d.x1 - d.x0) * 0.125))
          ...
          g.append('rect')
            ...
            .attr('rx', 15)
            .attr('ry', 15)
            .on('click', () => PlayShort(getParentsNotes(d), 0, getParents(d), axis))

          g.append('text').text('Path')
            .attr('class', 'pointer')
            .attr('fill', 'white')
            .attr('id', 'playbuttonsankey')
            .attr('x', ((d.x1 - d.x0) * 0.35) + d.x0 + ((d.x1 - d.x0) * 0.125))
            .attr('y', d.y0 + margin.top * 3 / 4)
            .on('click', () => PlayShort(getParentsNotes(d), 0, getParents(d), axis))
          ...
          g.append('text').text('Add').attr('class', 'pointer').attr('fill', 'white')
            .attr('x', d.x0 + ((d.x1 - d.x0) * 0.125))
            .attr('y', d.y0 + margin.top * 3 / 4)
            .on('mouseup', () => setMeloShort(d.idr, d.id, newfill))
        }

        function deleteButtonSankey () {
          d3.selectAll('#buttonSankey').remove()
        }
        ...
        // sankey
          if (!sankeyExist) {
            ...
            d3.select('#nodeLink').append('rect').attr('height', h)
              .attr('width', w)
              .attr('rx', 2)
              .attr('ry', 2)
              .attr('fill', darkmode === 'LightMode' ? 'white' : '#333')
          ...
          if (!sankeyExist) {
            ...
            // .attr("fill-opacity", 0.1)
              .on('mouseenter', function (e, d) {
                ...
                showButtonSankey(d, svg, newfill, axis)
              })
              .on('mouseout', function (e, d) {
                const offset = 1
          ...
          // x and y scale from node heigth and width ...
            ...
            let cs = []
            for (let p = axis[graph.nodes[i].layer].pmax - 2; p > axis[graph.nodes[i].layer].pmin + 2; p--) {
              if (p % 12 === 0) {
              // +1  -2
                cs.push(p)
              }
        ...
        function createSimpleRollForIcicly (data, tx, ty, twidth, theight, color, axis, h, i, inlayer, lastlayer) {
          ...
          if (h > 0) {
            ...
            if (ty !== 0) { tmargin.top = tmargin.top / h } else if (ty === 0 && i !== 0) { ty -= 20 }
          }
          ...
          svg.attr('height', theight).attr('width', twidth)
            .attr('x', tx).attr('y', ty).attr('id', 'roll' + i)
          ...
          if (i === 0) {
            d3.select('#nodeLink').append('g').attr('id', 'tgice')
        ...
        */
        ...
        if (notes1.length > 0) {
          if (toggleState === 1 || !sankeyExist) {
            d3.select('#nodeLink').attr('width', width).attr('height', height)
            d3.select('#nodeLink').selectAll('*').remove()

            const svg = d3.select('#nodeLink').append('g').attr('id', 'plotsgroup')
          ...
          for (let i = 0; i < notes.length; i++) {
            ...
            if (notes[i].length === 0) {
              layerpmax[layer[i]] = 75
              layerpmin[layer[i]] = 45
              layerlmax[layer[i]] = 0
              layerlmin[layer[i]] = 0
            }
          }
          ...
          if (notes.length > 0 && toggleState === 1) {
            ...
            for (let i = 0; i < notes.length; i++) {
              ...
              if (notes[i].length > 0 && notes[i][0].quantizedEndStep !== 0 && !notes[i][0].pitch !== 0) { createSimpleRollForIcicly(notes[i], xofparent[i], yofparent[i], layerwidth[layer[i]], heigthofparent[i], color[i], axis, numberofChilds[parent[i]], i, indexinchildlayer[i], layer[i] === layercount - 1) }
            }
          }
    ...
    function changeMode () {
      if (mode === 'samePitchRangePerLayer') {
        mode = 'ownPitchRange'
        document.getElementById('NlMode').innerText = mode
          ...
          <div>
            <div width={widthdiv} height={heightBox} style={{ height: heightBox + 'px', width: widthdiv + 'px', overflow: 'hidden' }} className='content-tabs viewDiv'>
              <AppBar position='static' sx={{ backgroundColor: 'toolbar.main' }}>
                <Toolbar variant='dense' sx={{ backgroundColor: 'toolbar.main', height: heightBox * 0.015 }}>
                  <Typography color='secondary'>Node-Link</Typography>
                  <Box sx={{ flexGrow: 6 }} />
                  <Tooltip title='Sort nodes of the same level dependent on the metric' placement='left'>
                    <Select
                      label='metric' color='primary' variant='standard' autoWidth
                      defaultValue={metric}
                      onChange={e => { metric = e.target.value; sorted = true; that.render() }}
                      sx={{
                        width: 200,
                        color: 'text.main',
                        minWidth: 172,
                    ...
                    >
                      <MenuItem key={0} value={0}>Similarity desc</MenuItem>
                      <MenuItem key={5} value={5}>Similarity asc</MenuItem>
                      <MenuItem key={1} value={1}>Temperature desc</MenuItem>
                      <MenuItem key={2} value={2}>Temperature asc</MenuItem>
                      <MenuItem key={3} value={3}>Variance of jumps desc</MenuItem>
                      <MenuItem key={4} value={4}>Variance of jumps asc</MenuItem>
                      <MenuItem key={6} value={6}>Normal Relationships</MenuItem>
                    </Select>
                  </Tooltip>
                  <Box sx={{ flexGrow: 6 }} />
                  <IconButton onClick={() => closeView(1, false)} edge='start' color='inherit' aria-label='menu' sx={{ mr: 2 }}>
                    <CloseIcon color='secondary' />
                  </IconButton>
                </Toolbar>
              </AppBar>
              <div width={widthdiv} height={heightdiv} style={{ marginTop: '7px', height: heightdiv + 'px', width: widthdiv + 'px', overflow: 'auto' }}>
                <svg className='noview' id='nodeLink' />
              </div>
              <svg className='noview' id='selectOfNL' />
              {/* <select className='button' id='metric' onChange={(value) => { metric = parseInt(document.getElementById('metric').value); sorted = true; that.render() }}>
            ...
            <button className="button" id="NlMode" onClick={changeMode}>samePitchRangePerLayer</button>
